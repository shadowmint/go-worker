...
import _ "n/external/github.com/go-sql-driver/mysql"
import nsql "n/sql"
...
func (self *sql) Execute(query *nsql.Query) (nsql.Rowset, error) {
	...
	return nil, n.Fail(1, "Invalid query type; not supported")
}
...
func (self *sql) query(query string, values ...[]interface{}) (*rowset, error) {
	...
	if err != nil {
		return nil, n.Fail(1, "Error perfroming explicit SQL query \"%s\": %s", query, err.Error())
	}
